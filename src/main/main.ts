...
const createWindow = async () => {
  ...
  ipcMain.handle('expandWindow', () =>
    !mainWindow?.isMaximized() ? mainWindow?.maximize() : mainWindow.restore()
  );
  ipcMain.handle('openFolder', (_, pathFolder: string) => {
    console.log('открыть папку');
  ...
  ipcMain.handle('downloadFile', async (_, url) => {
    ...
    if (mainWindow) {
      try {
        ...
        const file = await download(mainWindow, url, {
        ...
        console.log(error);
        mainWindow?.webContents.send('stateDownload', false);
      }
    }
  });
